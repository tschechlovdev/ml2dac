...
#result_path = Path("src/Experiments/VaryingTrainingData/real_world/")
result_path = Path("real_world/")
...
#real_world_data_path = Path("src/Experiments/real_world_data")
real_world_data_path = Path("../real_world_data")

test_data = [x for x in os.listdir(real_world_data_path) if ".csv" in x]
print(test_data)

# test_data = df_test.iloc[0]
# Extract Meta-Features from training datasets
mfs_all_training_datasets = []
for X, name in zip([shape_sets[d_name][0] for d_name in df_train["dataset"]],
...
# Create tree that we use to query nearest neighbors
tree = KDTree(mfs_all_training_datasets)

components = {"All_Components": {"n_warmstarts": 50, "limit_cs": True, "pred_cvi": True},
              "No CVI_Selection": {"n_warmstarts": 50, "limit_cs": True, "pred_cvi": False},
              "No CS_Red": {"n_warmstarts": 50, "limit_cs": False, "pred_cvi": True},
              "No Warmstart": {"n_warmstarts": 0, "limit_cs": True, "pred_cvi": True},
              }

for run in range(runs):
    for component in components.keys():
        if run == 0 and component == "All_components":
            continue
        component_config = components[component]
        n_warmstarts = component_config["n_warmstarts"]
        limit_cs = component_config["limit_cs"]
        pred_cvi = component_config["pred_cvi"]

        for data in test_data:
            data_df = pd.read_csv(real_world_data_path / data)
            test_X = data_df.iloc[:, :-1].to_numpy()
            test_y = data_df.iloc[:, -1].to_numpy()
            ...
            #percent_training_data = list(range(10, 110, 10))
            # Don't use all
            percent_training_data = [10, 30, 50, 70, 90, 100]
            absolut_n_training_data = [round((float(x) / 100) * float(len(df_train))) for x in percent_training_data]
            print(absolut_n_training_data)

            # Extract Meta-Features from test dataset
            mf_names, mfs = extract_meta_features(test_X, mf_set)
            ...
            for n_training_percent, n_training_absolut in zip(percent_training_data, absolut_n_training_data):
                file_name = dataset_name
                path = result_path / f"{component} / "f"run_{run}" / f"n_training_data_{n_training_percent}"

                if (path/ file_name).exists():
                    print(f"skipping {dataset_name} for component {component}")
                ...
                final_training_data = selected_training_data_df.sort_values("distance",
                                                                            ascending=False).head(n_training_absolut)
                ...
                if pred_cvi:
                    ...
                    predicted_cvi = CVICollection.DENSITY_BASED_VALIDATION
                    additional_result_info["cvi"] = predicted_cvi.get_abbrev()
                ...
                optimizer_result_df["n_training"] = n_training_absolut
                optimizer_result_df["%training"] = n_training_percent
