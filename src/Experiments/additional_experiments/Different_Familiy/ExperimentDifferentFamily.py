...
n_loops = 100
limit_cs = True
result_path = Path("results_not_same_family")
...
for run in range(runs):
    ...
    for i in range(len(df_test)):
        ...
        selected_training_data_df = selected_training_data_df[selected_training_data_df["type"] != test_type]
        ...
        print(f"{selected_training_data_df['type'].unique()}")
        ...
        path = result_path / f"run_{run}"

        if not path.exists():
            path.mkdir(exist_ok=True, parents=True)

        if (path / file_name).is_file():
            print(f"Continue for dataset {dataset_name}")
        ...
        print(optimal_cvi_per_dataset["dataset"].unique())
        print(names_final_training_datasets)
